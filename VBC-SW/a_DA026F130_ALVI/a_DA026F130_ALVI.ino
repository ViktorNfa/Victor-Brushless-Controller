...
const float vp = 0.3;                     // Velocity control loop PROPORTIONAL gain value      - VP
...
const float velocity_limit = 20;          // Velocity limit [rad/s]                             - LV
...
float max_rotor_position = 12.5f;         // Max position [rad] the rotor is allowed to spin to; above this value, motor will freewheel 
float min_rotor_position = -12.5f;        // Min position [rad] the rotor is allowed to spin to; below this value, motor will freewheel 

// --- Auto endstop finding + sweep ---
const float homing_velocity = 6.28f;      // [rad/s]
const float iq_trip = 0.5f;               // [A] threshold to detect hard stop
const float sweep_velocity = 4.0f * homing_velocity; // [rad/s]
const float sweep_span_ratio = 0.80f;     // use 80% of travel (centered)
