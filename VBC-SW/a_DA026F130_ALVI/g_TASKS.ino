...
void autoSweepUpdate() {
  if (!rotor_limits_found) return;

  // Use velocity mode for a clean constant-speed sweep
  motor.controller = MotionControlType::velocity;

  // Reverse at window edges
  if (motor.shaft_angle >= sweep_high) sweep_dir = -1;
  else if (motor.shaft_angle <= sweep_low) sweep_dir = +1;
